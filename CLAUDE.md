...
| `kernels/int8_attention.cu` | **INT8 attention** (primary target): INT8 WMMA QK^T + FP16 WMMA PV, per-token scales, templated on `(TILE_KV, HEAD_DIM)`. **Prefill-only** (seq_q == seq_kv) |
| `kernels/int8_decode_attention.cu` | **INT8 decode attention** (seq_q == 1): split-KV flash-decoding (partial online-softmax per KV chunk + log-sum-exp combine), warp-per-(b,h,split), barrier-free. Entry `int8_decode_attention_forward`; HEAD_DIM ∈ {64,128}. Host dispatches best partial per head_dim: **D=64 → dp4a lane-per-position** (iter 15, 1.24–1.56×), **D=128 → split-KV lane-per-dim** (iter 14, 1.23×). Wins vs FP16 SDPA at all serving-scale (B≥32) shapes at half the KV bytes. `INT8_DECODE_DP4A=0` forces the original D=64 path (ablation; dp4a@128 is a do-not-retry, reg-pressure regression) |

...
| `kernels/int8_decode_attention.cu` | **INT8 decode attention** (seq_q == 1): split-KV flash-decoding (partial online-softmax per KV chunk + log-sum-exp combine), warp-per-(b,h,split), barrier-free. Entry `int8_decode_attention_forward`; HEAD_DIM ∈ {64,128}. Where the INT8 **KV-cache** byte win is realized (iter 14, wins @D=128) |

...
//
// Scales are plain Python floats; the binding allocates small device tensors
// internally to satisfy the kernel's const float* interface.
...
// ── INT8 Attention binding ─────────────────────────────────────────────
// Q, K, V : (batch, heads, seq_len, head_dim)  torch.int8  CUDA
// scale_*  : Python float (per-tensor symmetric scale)
// returns  : (batch, heads, seq_len, head_dim)  torch.int8  CUDA
torch::Tensor int8_attention_forward_torch(
    ...
    auto d_sQ = make_scale_tensor(scale_Q, Q.device());
    auto d_sK = make_scale_tensor(scale_K, Q.device());
    auto d_sV = make_scale_tensor(scale_V, Q.device());

    int8_attention_forward(
        Q.data_ptr<int8_t>(), K.data_ptr<int8_t>(), V.data_ptr<int8_t>(),
        out.data_ptr<int8_t>(),
        d_sQ.data_ptr<float>(), d_sK.data_ptr<float>(), d_sV.data_ptr<float>(),

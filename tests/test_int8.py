...
Loads INT8 CUDA kernels via torch.utils.cpp_extension, runs correctness
checks against dequantized FP16 baseline, then benchmarks against FP16
baseline and cuBLAS INT8 (torch._int_mm).
"""
...
# ── Load CUDA kernels via JIT ──────────────────────────────────────────
PROJECT_ROOT = os.path.abspath(os.path.join(os.path.dirname(__file__), ".."))
KERNEL_SOURCES = [
    os.path.join(PROJECT_ROOT, "kernels", "int8_bindings.cu"),
    os.path.join(PROJECT_ROOT, "kernels", "int8_attention.cu"),
    os.path.join(PROJECT_ROOT, "kernels", "int8_mlp.cu"),
    os.path.join(PROJECT_ROOT, "kernels", "quant_utils.cu"),
]
...
def main():
    ...
    print("\nCompiling INT8 kernels (this may take a minute on first run)...")
    int8_kernels = load(
        name="int8_kernels",
    ...
    # ── References (dequant → FP16 baseline) ────────────────────────────
    ...
    # ── Correctness ─────────────────────────────────────────────────────
    print("=== Correctness ===")
    attn_out = int8_kernels.int8_attention_forward(
        Q_int8.contiguous(), K_int8.contiguous(), V_int8.contiguous(),
        scale_Q, scale_K, scale_V
    )
    mlp_out = int8_kernels.int8_mlp_forward(
        x_int8.contiguous(), W1_int8.contiguous(), W2_int8.contiguous(),
    ...
    def run_int8_attn():
        return int8_kernels.int8_attention_forward(
            Q_int8, K_int8, V_int8, scale_Q, scale_K, scale_V
        )
    int8_attn_ms = benchmark(run_int8_attn)
    ...
    def run_int8_mlp():
        return int8_kernels.int8_mlp_forward(
            x_int8, W1_int8, W2_int8, scale_x, scale_W1, scale_W2
        )
    int8_mlp_ms = benchmark(run_int8_mlp)

    # cuBLAS INT8 comparison (torch._int_mm, 2D only, single GEMM)
